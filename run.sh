...
# Add current workspace directory to PYTHONPATH
export PYTHONPATH="$(pwd):$PYTHONPATH"

# 1. Generate dataset if missing
if [ ! -f "noc-copilot/data/synthetic_telemetry.csv" ]; then
    echo "==> Generating synthetic telemetry training dataset..."
    python3 -m noc-copilot.faultsim.generate_dataset
fi

# 2. Train model if missing
if [ ! -f "noc-copilot/data/xgboost_model.json" ]; then
    echo "==> Training predictive XGBoost classifier..."
    python3 -m noc-copilot.engine.model train
fi

# 3. Build FAISS index if missing
if [ ! -f "noc-copilot/data/faiss_index.bin" ]; then
    echo "==> Compiling local RAG vector index..."
    python3 -m noc-copilot.copilot.rag
fi

# Cleanup old database to start clean
if [ -f "noc-copilot/data/telemetry.db" ]; then
    echo "==> Resetting live telemetry database..."
    rm -f noc-copilot/data/telemetry.db
fi

echo "==> Starting backend services..."

# Start simulation daemon
python3 -m noc-copilot.faultsim.inject_fault > /tmp/noc_faultsim.log 2>&1 &
SIM_PID=$!
echo "  [OK] Telemetry Simulation daemon started (PID: $SIM_PID)"

# Start predictive engine API (Port 8000)
uvicorn noc-copilot.engine.api:app --host 0.0.0.0 --port 8000 > /tmp/noc_engine.log 2>&1 &
ENGINE_PID=$!
echo "  [OK] Predictive Engine API started on port 8000 (PID: $ENGINE_PID)"

# Start copilot API (Port 8001)
uvicorn noc-copilot.copilot.api:app --host 0.0.0.0 --port 8001 > /tmp/noc_copilot.log 2>&1 &
COPILOT_PID=$!
...
streamlit run noc-copilot/ui/app.py --server.port 8501 --server.address 0.0.0.0
